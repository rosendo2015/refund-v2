...
export function Input({ legend, type = "text", ...rest }: Props) {
    return (
        <fieldset className="flex flex-1 max-h-20 text-gray-200 focus-within:text-green-100">
            ...
            <input type={type} className="w-full  h-12 rounded-lg border border-gray-300 px-2 text-sm text-gray-100 bg-transparent outline-none focus:border-2 focus:border-green-100 placeholder:text-gray-300" {...rest} />
        </fieldset>
    );
}